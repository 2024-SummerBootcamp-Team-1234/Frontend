...
const posts: Post[] = [
    ...
    { title: '00님의 재판 결과', categories: '카테고리 10', judgement: '판결', judgementTitle: '자세한 상황 필요 10', judgementDescription: '판결 설명 10' },
];
...
const MultipleItems: React.FC = () => {
    ...
    return (
        // 배경화면과 요소 정렬 속성
        <div className="bg-postPageBg-image bg-cover bg-center h-screen flex flex-col justify-center relative">
            {/* 여기서부터 슬라이드 시작! */}
            {/* 슬라이더를 내가 설정한 세팅값으로 참조하겠다. Width는 100% */}
            <Slider ref={sliderRef} {...settings} className={'w-full max-h-[60rem] flex'}>
                {/* 임시 데이터 배열 mapping, 포스트 값과 인덱스 값 사용 */}
                {posts.map((post, index) => (
                    <div
                        key={index}
                        className={`max-w-[40rem] h-[40rem] my-auto rounded-3xl shadow-6xl transition-all duration-500 border-2 border-solid border-white
                                      ${activeSlide === index
