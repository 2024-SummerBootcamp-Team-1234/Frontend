...
const categories: Category[] = [
    ...
    { id: 2, name: '교통사고', description: '탈 것이 운행 중에 사람이나 다른 것과 충돌하는 교통 상의 사고' },
    { id: 3, name: '사기', description: '고의로 다른 사람을 속여 재산상의 이익을 취하는 행위' },
    { id: 4, name: '이혼', description: '부부가 합의 또는 재판에 의하여 혼인 관계를 인위적으로 소멸시키는 일' },
    { id: 5, name: '성범죄', description: '성적 행위를 강요하거나 성적 자기결정권을 침해하는 행위' },
    { id: 6, name: '디지털 프라이버시', description: '인터넷과 디지털 환경에서 프라이버시와 데이터 보호 문제를 다룸' },
    { id: 7, name: '의료 과실', description: '의료 제공자의 과실 또는 의료 행위로 인한 환자의 손해에 대해 법적 책임' },
    { id: 8, name: '학교 폭력', description: '학생 간의 신체적, 정신적 폭력 행위에 대해 학교와 관련 법률에 따라 처벌과 예방 조치를 요구하는 법적 문제' },
    { id: 9, name: '폭행', description: '다른 사람의 신체를 고의로 공격하거나 위협하여 상해를 입히는 행위' },
    { id: 10, name: '환경오염', description: '인간의 활동으로 인해 대기, 토양, 수질 등을 오염시켜 생태계와 인간 건강에 해를 끼치는 행위' },
];
...
const CategoryPage: React.FC = () => {
    ...
    return (
        <div className="bg-black bg-category-image bg-cover bg-center min-h-screen flex flex-col items-start p-4 relative">
            ...
            <button className= "bg-arrow-image bg-no-repeat bg-contain w-full h-[4.8vh] mx-auto mt-8 mb-4" style={{maxWidth:'107rem'}} onClick={handleButtonClick}></button>

            {/* 이미지 + Category + Skip > */}
            <div className= "w-full mx-auto text-white flex justify-start items-center" style={{maxWidth:'95rem'}}>
            ...
            <p className="w-full mx-auto mb-8 text-2xl text-white text-start font-sans font-normal" style={{maxWidth:'95rem'}}>
